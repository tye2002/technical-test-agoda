...
export class AgodaHotelDetailPage {
  ...
  constructor(page: Page) {
    ...
    this.priceBoxes = page.locator(
      '[data-element-name="fpc-room-price"]'
    );
    this.soldOutBox = page.locator(
      '[data-element-name="mob-property-sold-out-all"]'
    );
  }
  ...
  async getHotelPrice(): Promise<string> {
    ...
    const priceText =
      (await priceBox.textContent())?.trim() ?? '';
    return priceText;
  }
  ...
  async verifyHotelPriceDisplayed(): Promise<string> {
    ...
    const isSoldOut = await this.soldOutBox
      .isVisible()
      .catch(() => false);
    expect(
      isSoldOut,
    ...
    await expect(firstPriceBox).toBeVisible({
      timeout: 30000
    });
